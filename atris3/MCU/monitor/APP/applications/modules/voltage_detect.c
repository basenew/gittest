...
uint8_t cmd[16][2] = {{0x10,0x80},  //ch0
					  {0x11,0x00},
					  {0x11,0x80},
					  {0x12,0x00},
					  {0x12,0x80},
...
static uint32_t g_voltage_voldata_time_interval = 2000;
...
static void voltage_adc_print(void)
{
    ...
    if (g_voltage_adc_print_flag != NO)
    {
        if (os_gettime_ms() - s_print_timer >= g_voltage_adc_time_interval)
        {
			...
			rt_kprintf("vol adc:ch0-%d  ch1-%d  ch2-%d  ch3-%d  ch4-%d  ch5-%d  ch6-%d  ch7-%d  ch8-%d  ch9-%d  ch10-%d  ch11-%d  ch12-%d  ch13-%d\n", 
						vol_adc_value[0], vol_adc_value[1], vol_adc_value[2], vol_adc_value[3], vol_adc_value[4], vol_adc_value[5], vol_adc_value[6], \
						vol_adc_value[7], vol_adc_value[8], vol_adc_value[9], vol_adc_value[10], vol_adc_value[11], vol_adc_value[12], vol_adc_value[13]);
...
static void voltage_voldata_print(void)
{
    ...
    if (g_voltage_voldata_print_flag != NO)
    {
        if (os_gettime_ms() - s_print_timer >= g_voltage_voldata_time_interval)
        {
			...
			rt_kprintf("vol_val:ch0-%d ch1-%d ch2-%d ch3-%d ch4-%d ch5-%d ch6-%d ch7-%d ch8-%d ch9-%d ch10-%d ch11-%d ch12-%d ch13-%d\n", 
						vol_converted_value[0], vol_converted_value[1], vol_converted_value[2], vol_converted_value[3], vol_converted_value[4],  \
						vol_converted_value[5], vol_converted_value[6], vol_converted_value[7], vol_converted_value[8], vol_converted_value[9],  \
						vol_converted_value[10], vol_converted_value[11], vol_converted_value[12], vol_converted_value[13]);
        } else if (os_gettime_ms() < s_print_timer) {
            s_print_timer = os_gettime_ms();
        }
    }
...
static void voltage_convert(uint16_t vol_data, uint8_t _ch)
{
	...
	{
		case 0:
			vol_converted_value[_ch] = vol_data * 67/20;	//(2K+4.7K)/2K
			break;
		
		case 1:
			vol_converted_value[_ch] = vol_data * 12/2;		//(2K+10K)/2K
			break;
		...
		case 3:
			vol_converted_value[_ch] = vol_data * 22/2;		//(2K+20K)/2K
			break;
		...
		case 9:
			vol_converted_value[_ch] = vol_data * 80/33;	//(3.3K+4.7K)/3.3K
			break;
		
		case 10:
			vol_converted_value[_ch] = vol_data * 247/47;	//(4.7K+20K)/4.7K
			break;
