...
#define WEBCLIENT_RESPONSE_BUFSZCCCC 1024
#define WEBCLIENT_HEADER_BUFSZCCCC 1024
...
int webclient_get_file(const char* URI, const char* filename)
{
    ...
    session = webclient_session_create(WEBCLIENT_HEADER_BUFSZCCCC);
    if(session == RT_NULL)
    {
        rc = -WEBCLIENT_NOMEM;
        goto __exit;
    ...
    ptr = (unsigned char *) web_malloc(WEBCLIENT_RESPONSE_BUFSZCCCC);
    if (ptr == RT_NULL)
    {
    ...
    if (session->content_length < 0)
    {
        while (1)
        {
            length = webclient_read(session, ptr, WEBCLIENT_RESPONSE_BUFSZCCCC);
            if (length > 0)
            {
                write(fd, ptr, length);
                total_length += length;
    ...
    else
    {
        for (offset = 0; offset < (size_t) session->content_length;)
        {
            length = webclient_read(session, ptr,
                    session->content_length - offset > WEBCLIENT_RESPONSE_BUFSZCCCC ?
                            WEBCLIENT_RESPONSE_BUFSZCCCC : session->content_length - offset);
        ...
        const char* form_data)
{
    ...
    buffer = (unsigned char *) web_calloc(1, WEBCLIENT_RESPONSE_BUFSZCCCC);
    if (buffer == RT_NULL)
    {
    ...
    header = (char *) web_calloc(1, WEBCLIENT_HEADER_BUFSZCCCC);
    if (header == RT_NULL)
    {
    ...
    buffer_ptr += rt_snprintf((char*) buffer_ptr,
            WEBCLIENT_RESPONSE_BUFSZCCCC - (buffer_ptr - buffer), "--%s\r\n", boundary);

    buffer_ptr += rt_snprintf((char*) buffer_ptr,
            WEBCLIENT_RESPONSE_BUFSZCCCC - (buffer_ptr - buffer),
    ...
    header_ptr += rt_snprintf(header_ptr,
            WEBCLIENT_HEADER_BUFSZCCCC - (header_ptr - header),
            "Content-Length: %d\r\n", length);
    header_ptr += rt_snprintf(header_ptr,
            WEBCLIENT_HEADER_BUFSZCCCC - (header_ptr - header),
    ...
    while (1)
    {
        length = read(fd, buffer, WEBCLIENT_RESPONSE_BUFSZCCCC);
        if (length <= 0)
        {
            break;
        }
    ...
    /* send last boundary */
    rt_snprintf((char*) buffer, WEBCLIENT_RESPONSE_BUFSZCCCC, "\r\n--%s--\r\n", boundary);
    ...
    if (resp_data_len > 0)
    {
        ...
        rt_memset(buffer, 0x00, WEBCLIENT_RESPONSE_BUFSZCCCC);
        do
        {
            bytes_read = webclient_read(session, buffer,
                resp_data_len < WEBCLIENT_RESPONSE_BUFSZCCCC ? resp_data_len : WEBCLIENT_RESPONSE_BUFSZCCCC);
            if (bytes_read <= 0)
            {
                break;
            }
...
int wpost(int argc, char** argv)
{
    ...
    webclient_post_file(argv[1], argv[2], "name=\"a.txt\"; filename=\"1.txt\"");
    return 0;
}
...
MSH_CMD_EXPORT(wpost, Post file by URI: wpost <URI> <filename>.);
