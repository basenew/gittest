...
const char* ConfigUrl = "chassis_controller/config.yaml";
const char* AppUrl    = "chassis_controller/app.rbl";
const char* BlUrl     = "chassis_controller/bootloader.rbl";
...
if(DF_THREAD_STATIC_MEMORY == 0){   	
    ...
    static struct rt_thread ota_thread;
    ALIGN(RT_ALIGN_SIZE)
    static char ota_thread_stack[TASK_STACK_SIZE_OTA]; 
    	rt_err_t result = RT_EOK;
    result = rt_thread_init(&ota_thread,
                            "ota",
                            task_main, RT_NULL,
                            &ota_thread_stack[0], sizeof(ota_thread_stack),
                            TASK_PRIORITY_OTA, 20);
