...
if(DF_THREAD_STATIC_MEMORY == 0){       
    ...
    static struct rt_thread sw_thread;
    ALIGN(RT_ALIGN_SIZE)
    static char sw_thread_stack[TASK_STACK_SIZE_SW]; 
    	rt_err_t result = RT_EOK;
    result = rt_thread_init(&sw_thread,
                            "sw",
                            sw_process, RT_NULL,
                            &sw_thread_stack[0], sizeof(sw_thread_stack),
                            TASK_PRIORITY_SW, 20);
