...
if(DF_THREAD_STATIC_MEMORY == 0){    
    ...
    static struct rt_thread log_thread;
    ALIGN(RT_ALIGN_SIZE)
    static char log_thread_stack[TASK_STACK_SIZE_LOG]; 
    	rt_err_t result = RT_EOK;
    result = rt_thread_init(&log_thread,
                            "log",
                            task_main, RT_NULL,
                            &log_thread_stack[0], sizeof(log_thread_stack),
...
//------------------------------------------------------------------------------------------------------------------
extern int webclient_post_file(const char* URI, const char* filename, const char* form_data);
int log_upload(void)
{
    int ret = -1;
    int file_cur = 0;
    ...
    rt_kprintf("g_file_cnts: %d \n", g_file_cnts);
    
    for (int i = 0; i < g_file_cnts; i++) 
    {
        rt_thread_mdelay(50);
        
        file_cur = g_file_array[i];
        
        rt_memset(filename, 0, sizeof(filename));
        rt_sprintf(filename, "%02d-%02d-%04d-%02d", file_cur%10000/100, file_cur%1000000/10000, file_cur/1000000, file_cur%100);

        rt_memset(fullpath, 0, sizeof(fullpath));
        rt_sprintf(fullpath, "%s/%s", LOG_DIR, filename);
        
        rt_memset(formdata, 0, sizeof(formdata));
        rt_sprintf(formdata, "name=\"%s\"; filename=\"%s\"", filename, filename);
        
        rt_kprintf("file fullpath: %s \n", fullpath);
        webclient_post_file("http://10.20.18.2/hfs/upload", fullpath, formdata);
    }
    ret = 0;
__exit:
    // resume recording logs
    log_obj.isinit = 1;
    return ret;
}
...
static void logupload(uint8_t argc, char **argv)
{
    log_upload();
}
MSH_CMD_EXPORT(logupload, upload log files.);
