...
#define INAK_TIMEOUT_STD      ((uint32_t)0x0000FFFF)
/* Time out for SLAK bit */
#define SLAK_TIMEOUT_STD      ((uint32_t)0x0000FFFF)
...
uint8_t CAN_Init(CAN_TypeDef* CANx, CAN_InitTypeDef* CAN_InitStruct)
{
  ...
  while (((CANx->MSR & CAN_MSR_INAK) != CAN_MSR_INAK) && (wait_ack != INAK_TIMEOUT_STD))
  {
    wait_ack++;
  }
...
uint8_t CAN_OperatingModeRequest(CAN_TypeDef* CANx, uint8_t CAN_OperatingMode)
{
  ...
  uint32_t timeout = INAK_TIMEOUT_STD; 
...
{
  uint32_t wait_slak = SLAK_TIMEOUT_STD;
