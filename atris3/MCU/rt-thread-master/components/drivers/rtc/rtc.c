...
rt_err_t set_date(rt_uint32_t year, rt_uint32_t month, rt_uint32_t day)
{
    ...
    ret = rt_device_control(device, RT_DEVICE_CTRL_RTC_SET_TIME, &tm_new);
...
rt_err_t rtc_sync_time(struct tm *p_tm)
{
    rt_device_t device;
    rt_err_t ret = -RT_ERROR;
    ...
    /* update to RTC device. */
    ret = rt_device_control(device, RT_DEVICE_CTRL_RTC_SET_TIME, p_tm);
