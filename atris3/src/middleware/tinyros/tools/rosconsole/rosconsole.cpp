...
#define LOG_ROTATION_SIZE    (40*1024*1024) //10MB
#define LOG_ROTATION_FILES0  (100)
#define LOG_ROTATION_FILES1  (5)
...
static void messageCb(const tinyros::tinyros_msgs::Log& l) {
  ...
  } else if(l.level == tinyros::tinyros_msgs::Log::ROSDIAG && option_ == OPTION_D) {
    if (spdlog::get("diag")) spdlog::get("diag")->info("{0}", l.msg);
  }
}
...
static void init_log_environment() {
  ...
  if (option_ == OPTION_P) {
    ...
    if (option_ == OPTION_D) {
      log_sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt> (
        param_ + "/tinyrosapps.log", LOG_ROTATION_SIZE, LOG_ROTATION_FILES0);
    } else {
  ...
  if (option_ == OPTION_D) {
    auto diag_sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(
      param_ + "/tinyrosdiagnostics.log", LOG_ROTATION_SIZE, LOG_ROTATION_FILES1);
    diag_sink->set_level(spdlog::level::trace);
    auto diag_logger = std::make_shared<spdlog::logger>("diag", diag_sink);
    diag_logger->set_level(spdlog::level::trace);
    diag_logger->flush_on(spdlog::level::trace);
    diag_logger->set_pattern("[%Y-%m-%d %H:%M:%S.%e] [%l] %v");
    spdlog::register_logger(diag_logger);
  }
}
