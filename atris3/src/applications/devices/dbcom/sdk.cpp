...
static void sdk_remove_event(struct sdk_connection *conn, int index);
...
static int send_length(struct sdk_connection *conn, int len) {
	char data[4];
	int written;
	int towrite;
...
static int readLen(struct sdk_connection *conn)
{
	char data[4];
...
static void sdk_handle_events(struct sdk_connection *conn, struct sdk_result *result) {
	if (conn->max_events > 0) {
		int i;
		char *key = sdk_get_tag(result);
		if (key == NULL) {
			/* Event with no tag */
			return;
		}
...
int sdk_runloop_once(struct sdk_connection *conn, void (*callback)(struct sdk_result *result)) {
	...
	if (conn->type != sdk_EVENT) {
		fprintf(stderr, "Warning! Connection type was not set to sdk_EVENT. Forcing change.\n");
		sdk_set_type(conn, sdk_EVENT);
	}
	...
	if (conn->expected_length == 0) {
		...
		if (conn->expected_length > 0) {
			...
			sdk_runloop_once(conn, callback);
		} else if (conn->expected_length == 0) {
			// Sentence done
			// call callback
			struct sdk_result *res = conn->event_result;
		...
		if (got == to_read) {
			...
			if (conn->event_result == NULL) {
				conn->event_result = (struct sdk_result *)malloc(sizeof(struct sdk_result));
				if (conn->event_result == NULL) {
					fprintf(stderr, "Error allocating memory for event result\n");
					exit(1);
				}
				memset(conn->event_result, 0, sizeof(struct sdk_result));
				conn->event_result->sentence = sdk_sentence_new();
			...
			sdk_sentence_add(res->sentence, (char *)conn->buffer);
...
struct sdk_connection *sdk_connect(char *address, int port) {
	...
	struct sdk_connection *conn = (struct sdk_connection *)malloc(sizeof(struct sdk_connection));
...
void sdk_result_free(struct sdk_result *result) {
	sdk_sentence_free(result->sentence);
...
struct sdk_result *sdk_read_packet(struct sdk_connection *conn) {
	struct sdk_result *ret = (struct sdk_result *)malloc(sizeof(struct sdk_result));
	int len;
	...
	memset(ret, 0, sizeof(struct sdk_result));
	ret->done = 0;
	ret->re = 0;
	ret->trap = 0;
	ret->fatal = 0;
	ret->sentence = sdk_sentence_new();
	...
	do {
		...
		if (len > 0) {
			...
			buffer[len] = '\0';
			sdk_sentence_add(ret->sentence, buffer);
		}
		free(buffer);
...
struct sdk_sentence *sdk_sentence_new() {
	struct sdk_sentence *res = (struct sdk_sentence *)malloc(sizeof(struct sdk_sentence));
	res->words = 0;
...
static struct sdk_sentence *sdk_va_to_sentence(va_list ap, char *first, char *second) {
	int i = 0;
	struct sdk_sentence *res = sdk_sentence_new();
...
int sdk_send_command_args(struct sdk_connection *conn, char **args, int num) {
	int i = 0, len = 0;
	char *arg;
	if (num == 0) return 0;
...
int sdk_send_sentence(struct sdk_connection *conn, struct sdk_sentence *sentence) {
	if (conn == NULL || sentence == NULL) {
		return 0;
	}

	return sdk_send_command_args(conn, sentence->word, sentence->words);
}

static int sdk_send_command_va(struct sdk_connection *conn, char *extra, char *command, va_list ap) {
	struct sdk_sentence *sentence;
	int result;
	
	sentence = sdk_va_to_sentence(ap, command, extra);
	result = sdk_send_sentence(conn, sentence);
	sdk_sentence_free(sentence);
	return result;
}

void sdk_add_event(struct sdk_connection *conn, struct sdk_event *event) {
	int idx = 0;
	char isnew = 0;

	if (conn->events == NULL) {
		conn->max_events = 1;
		conn->events = (struct sdk_event **)malloc(sizeof(struct sdk_event **));
		idx = 0;
		isnew = 1;
	} else {
		int i;
		...
		if (idx == -1) {
			isnew = 1;
			idx = conn->max_events++;
			conn->events = (struct sdk_event **)realloc(conn->events, sizeof(struct sdk_event **) * conn->max_events);
			if (conn->events == NULL) {
				fprintf(stderr, "Error allocating memory\n");
				exit(1);
			}
	...
	if (isnew) {
		conn->events[idx] = (struct sdk_event *)malloc(sizeof(struct sdk_event));
		if (conn->events[idx] == NULL) {
			fprintf(stderr, "Error allocating memory\n");
		}
	}
	memcpy(conn->events[idx], event, sizeof(struct sdk_event));
...
int sdk_cancel(struct sdk_connection *conn, int id) {
	...
	int returnval;
	struct sdk_result *res;
	int was_event = conn->type == sdk_EVENT;
	
	snprintf(iddata, 1024, "=tag=%d", id);
	if (was_event) {
		sdk_set_type(conn, sdk_SIMPLE);
	}
	res = sdk_send_command_wait(conn, "/cancel", iddata, NULL);
	if (was_event) {
		sdk_set_type(conn, sdk_EVENT);
	}
	if (res == NULL) {
...
int sdk_send_command_cb(struct sdk_connection *conn, void (*callback)(struct sdk_result *result), char *command, ...) {
	int result;
	int id;
	struct sdk_event *event = (struct sdk_event *)malloc(sizeof(struct sdk_event));
	char extra[120];
	va_list ap;
	...
	sdk_add_event(conn, event);
	free(event);

	va_start(ap, command);
	result = sdk_send_command_va(conn, extra, command, ap);
	va_end(ap);
...
int sdk_send_sentence_cb(struct sdk_connection *conn, void (*callback)(struct sdk_result *result), struct sdk_sentence *sentence) {
	...
	sdk_sentence_add(sentence, extra);
	result = sdk_send_sentence(conn, sentence);
...
int sdk_send_command(struct sdk_connection *conn, char *command, ...) {
	...
	va_list ap;
	va_start(ap, command);
	result = sdk_send_command_va(conn, NULL, command, ap);
	va_end(ap);
	return result;
}
...
int sdk_login(struct sdk_connection *conn, char *username, char *password) {
	...
	res = sdk_send_command_wait(conn, "/login",
		userWord,
		passWord,
		NULL
	);

	challenge = sdk_get(res, "=ret");
	if (challenge == NULL) {
		return 1;
	}
	sdk_result_free(res);
