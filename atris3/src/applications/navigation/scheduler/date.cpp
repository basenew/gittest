...
namespace sched
{
	...
	std::string Date::Format() const
	{
		...
		if (WithTime == true)
			sprintf(&Temp[10], " %.2d:%.2d:%.2d", Data[HOUR], Data[MINUTE], Data[SECOND]);
	...
	void Date::ConvertTimestampToDate(long timestamp)
	{
		timestamp = timestamp * 0.001; //ms -> s
		const time_t rawtime = (const time_t)timestamp;
		tm *dt = localtime(&rawtime);

		Data[YEAR] = dt->tm_year + 1900;
		Data[MONTH] = dt->tm_mon + 1;
		Data[DAY] = dt->tm_mday;
		Data[HOUR] = dt->tm_hour;
		Data[MINUTE] = dt->tm_min;
		Data[SECOND] = dt->tm_sec;

		WithTime = true;
	}

	long Date::ConvertDateToTimestamp()
	{
		time_t rawtime;
		time(&rawtime);
		tm *dt = localtime(&rawtime);

		dt->tm_year = Data[YEAR] - 1900;
		dt->tm_mon = Data[MONTH] - 1;
		dt->tm_mday = Data[DAY];
		dt->tm_hour = Data[HOUR];
		dt->tm_min = Data[MINUTE];
		dt->tm_sec = Data[SECOND];
